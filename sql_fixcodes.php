...
//
$code_query = mysql_query("SELECT * FROM fund_types");
$codes = array();
while($code_array = mysql_fetch_array($code_query)){
	$codes[] = $code_array;
}
//print_r($codes);
$acct_query = mysql_query("SELECT * FROM accounts WHERE acct_type = 0 OR acct_company = 0");
while($acct_array = mysql_fetch_array($acct_query)){
	$adder = array();
	$type_code_old = $acct_array['acct_type_code'];
	$type_company_old = $acct_array['acct_code_code'];
	//Get correct type_code id
	$type_code = 0;
	foreach($codes as $code){
		if($code['type'] == "T" && $code['code'] == $type_code_old){
			$type_code = $code['id'];
		}
	}
	$adder[] = array('acct_type',$type_code,"num");
	//Get correct type_company
	$type_company = 0;
	foreach($codes as $code){
		if($code['type'] == "C" && $code['code'] == $type_company_old){
			$type_company = $code['id'];
		}
	}
	$adder[] = array('acct_company',$type_company,"num");
	//db_update("accounts",$adder,$acct_array['id']);
	echo($type_code." / ".$type_company."<br/>");
}
/*
